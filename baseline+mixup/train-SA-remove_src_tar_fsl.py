...
for iDataSet in range(nDataSet) :
    ...
    for episode in range(EPISODE) :
        ...
        mixuped_mapped_query_data, lam = utils.mixup_data(mapped_query_src, mapped_query_tar, alpha=20.0) # (171, 100, 7, 7)
        ...
        total_hit_src += torch.sum(torch.argmax(logits_mixup_src, dim=1).cpu() == query_label_src).item()
        ...
        total_hit_tar += torch.sum(torch.argmax(logits_mixup_tar, dim=1).cpu() == query_label_tar).item()
        total_num_tar += query_tar.shape[0]
        acc_tar = total_hit_tar / total_num_tar

        if (episode + 1) % 100 == 0:
            # tensor.item() 把张量转换为python标准数字返回，仅适用只有一个元素的张量。
            logger.info('episode: {:>3d}, mixup_loss: {:6.4f}, loss: {:6.4f}, acc_src: {:6.4f}, acc_tar: {:6.4f}'.format(
                episode + 1,
                # f_loss.item(),
                mixup_loss.item(),
                loss.item(),
