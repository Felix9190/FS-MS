...
class DPGN(nn.Module):
    ...
    def forward(self, middle_node, point_node, distribution_node, distribution_edge, point_edge):
        ...
        for l in range(self.generation):
            ...
            # print("point_edge.shape=",point_edge.shape,"node_similarity_l2.shape=",node_similarity_l2.shape)
            # point_edge.shape= torch.Size([1, 180, 180]) node_similarity_l2.shape= torch.Size([1, 180, 180])
            distribution_node = self._modules['point2distribution_generation_{}'.format(l)](point_edge, distribution_node)
            # print("distribution_node.shape=", distribution_node.shape)
            # distribution_node.shape= torch.Size([1, 180, 9])
            ...
            # print("distribution_edge.shape=", distribution_edge.shape)
            # distribution_edge.shape= torch.Size([1, 180, 180])
            point_node = self._modules['distribution2point_generation_{}'.format(l)](distribution_edge, point_node)
            # point_node.shape= torch.Size([1, 180, 128])
            # print("point_node.shape=",point_node.shape)
            ...
            distribution_nodes.append(distribution_node) # [1,180,9]
